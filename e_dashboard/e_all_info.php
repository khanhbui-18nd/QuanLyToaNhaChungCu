          ...
          <div class="box-body">
            <table style="font-size:13px;" class="table sakotable table-bordered table-striped dt-responsive">
              ...
              <tbody>
                ...
                $mid = $_GET['mid'];
                $yid = $_GET['yid'];
                $result = mysqli_query($conn, "SELECT *, m.month_name FROM tbl_add_employee_salary_setup es INNER JOIN tbl_add_month_setup m ON m.m_id = es.month_id WHERE es.emp_name = '$emp_id' AND es.month_id='$mid' AND es.xyear='$yid' ORDER BY es.emp_id DESC");
