...
if(isset($_POST['txtMonthName'])){
	if($_POST['hdnSpid'] == '0'){
		$sql = "INSERT INTO `tbl_add_month_setup`(`month_name`) VALUES ('".$conn->real_escape_string($_POST['txtMonthName'])."')";
		$conn->query($sql);
		$url = WEB_URL . 'setting/month_setup.php?m=add';
		header("Location: $url");
	} else {
		$sql_update = "UPDATE `tbl_add_month_setup` SET month_name = '".$conn->real_escape_string($_POST['txtMonthName'])."' WHERE m_id = '".(int)$_POST['hdnSpid']."'";
...
if(isset($_GET['spid']) && $_GET['spid'] != ''){
	$result_location = $conn->query("SELECT * FROM tbl_add_month_setup WHERE m_id = '" . (int)$_GET['spid'] . "'");
  ...
  <div class="col-md-12">
    ...
    <div class="box box-info">
      ...
      if(isset($_GET['delid']) && $_GET['delid'] != '' && $_GET['delid'] > 0){
          $sqlx = "DELETE FROM `tbl_add_month_setup` WHERE m_id = " . (int)$_GET['delid'];
          $conn->query($sqlx);
          $delinfo = 'block';
      }
          ...
          <div class="box box-info">
            ...
            <div class="box-body">
              <table class="table sakotable table-bordered table-striped dt-responsive">
                ...
                <tbody>
                  <?php
                  $result = $conn->query("SELECT * FROM tbl_add_month_setup ORDER BY m_id ASC");
                  while($row = $result->fetch_assoc()){ ?>
                  <tr>
                    <td><?php echo $row['month_name']; ?></td>
                    <td>
                      <a class="btn btn-success" data-toggle="tooltip" href="javascript:;" onclick="$('#employee_view_<?php echo $row['m_id']; ?>').modal('show');" data-original-title="<?php echo $_data['view_text']; ?>"><i class="fa fa-eye"></i></a> 
                      <a class="btn btn-primary" data-toggle="tooltip" href="<?php echo WEB_URL; ?>setting/month_setup.php?spid=<?php echo $row['m_id']; ?>" data-original-title="<?php echo $_data['edit_text']; ?>"><i class="fa fa-pencil"></i></a> 
                      <a class="btn btn-danger" data-toggle="tooltip" onclick="deleteMonth(<?php echo $row['m_id']; ?>);" href="javascript:;" data-original-title="<?php echo $_data['delete_text']; ?>"><i class="fa fa-trash-o"></i></a>
