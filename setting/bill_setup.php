...
if (isset($_POST['txtBillType'])) {
    if ($_POST['hdnSpid'] == '0') {
        $sql = "INSERT INTO `tbl_add_bill_type`(`bill_type`) VALUES (?)";	
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $_POST['txtBillType']);
        $stmt->execute();
        $stmt->close();
        ...
        $sql_update = "UPDATE `tbl_add_bill_type` SET bill_type = ? WHERE bt_id = ?";
        $stmt = $conn->prepare($sql_update);
        $stmt->bind_param("si", $_POST['txtBillType'], $_POST['hdnSpid']);
        $stmt->execute();
        $stmt->close();
...
if (isset($_GET['spid']) && $_GET['spid'] != '') {
    $stmt = $conn->prepare("SELECT * FROM tbl_add_bill_type WHERE bt_id = ?");
      ...
      <form onSubmit="return validateMe();" action="<?php echo $form_url; ?>" method="post" enctype="multipart/form-data">
        <div class="box-body">
          <div class="form-group">
            ...
            <input type="text" name="txtBillType" required value="<?php echo $bill_type;?>" id="txtBillType" class="form-control" />
          </div>
...
if (isset($_GET['delid']) && $_GET['delid'] != '' && $_GET['delid'] > 0) {
    $stmt = $conn->prepare("DELETE FROM `tbl_add_bill_type` WHERE bt_id = ?");
              ...
              <table class="table sakotable table-bordered table-striped dt-responsive">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <a class="btn btn-success" data-toggle="tooltip" href="javascript:;" onclick="$('#employee_view_<?php echo $row['bt_id']; ?>').modal('show');" data-original-title="<?php echo $_data['view_text'];?>">
                        <i class="fa fa-eye"></i>
                      </a> 
                      <a class="btn btn-primary" data-toggle="tooltip" href="<?php echo WEB_URL;?>setting/bill_setup.php?spid=<?php echo $row['bt_id']; ?>" data-original-title="<?php echo $_data['edit_text'];?>">
                        <i class="fa fa-pencil"></i>
                      </a> 
                      <a class="btn btn-danger" data-toggle="tooltip" onclick="deleteBillType(<?php echo $row['bt_id']; ?>);" href="javascript:;" data-original-title="<?php echo $_data['delete_text'];?>">
                        ...
                        <div class="modal-dialog">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              ...
                              <div class="row">
                                <div class="col-xs-12"> 
                                  <b><?php echo $_data['text_3'];?> :</b> <?php echo $row['bill_type']; ?><br/>
                                </div>
                              </div>
                            </div>
                          </div>
