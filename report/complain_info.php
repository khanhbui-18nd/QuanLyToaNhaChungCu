                        ...
                        <div class="box-body">
                            <table style="font-size:13px;" class="table sakotable table-bordered table-striped dt-responsive">
                                ...
                                <tbody>
                                    ...
                                    $yid = mysqli_real_escape_string($conn, $_GET['yid']);
                                    
                                    $result = mysqli_query($conn, "SELECT *, m.month_name FROM tbl_add_complain c INNER JOIN tbl_add_month_setup m ON m.m_id = c.c_month WHERE c.c_date='$vid' AND c.c_month='$mid' AND c.c_year='$yid' AND c.branch_id='" . (int)$_SESSION['objLogin']['branch_id'] . "'");
