        ...
        <div class="box box-info">
          ...
          <div class="box-body">
            <table style="font-size:13px;" class="table sakotable table-bordered table-striped dt-responsive">
              ...
              <tbody>
            <?php
            // Truy vấn dữ liệu từ bảng tbl_add_complain
            $stmt = $conn->prepare("SELECT *, m.month_name FROM tbl_add_complain c INNER JOIN tbl_add_month_setup m ON m.m_id = c.c_month WHERE c.c_year=? AND c.branch_id=?");
            $stmt->bind_param("ii", $_GET['yid'], $_SESSION['objLogin']['branch_id']);
            $stmt->execute();
