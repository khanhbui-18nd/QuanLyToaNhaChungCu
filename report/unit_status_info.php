        ...
        <div class="box box-info">
          ...
          <div class="box-body">
            <table style="font-size:13px;" class="table sakotable table-bordered table-striped dt-responsive">
              ...
              <tbody>
            <?php
            $result = mysqli_query($conn, "SELECT *, f.floor_no as ffloorno 
                                            FROM tbl_add_unit u 
                                            INNER JOIN tbl_add_floor f ON f.fid = u.floor_no 
                                            WHERE u.status='" . mysqli_real_escape_string($conn, $_GET['usid']) . "' 
                                            AND u.branch_id = '" . (int)$_SESSION['objLogin']['branch_id'] . "'");
            while($row = mysqli_fetch_array($result)){?>
                <tr>
                    <td><?php echo $row['ffloorno']; ?></td>
                    <td><?php echo $row['unit_no']; ?></td>
                    <td><?php echo ($row['status'] == '1') ? $_data['text_5'] : $_data['text_6']; ?></td>
                </tr>
...
<!-- /.row -->
<div align="center">
    <a class="btn btn-primary btn_save" onClick="javascript:printContent('printable','Unit Status Report');" href="javascript:void(0);"><?php echo $_data['print'];?></a>
</div>
</body>
</html>
